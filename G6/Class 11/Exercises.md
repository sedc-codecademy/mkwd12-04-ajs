...
* StartSubject - accepts Subject object and adds it to the CurrentSubject property but only if the student has an Academy object in the Academy property and that subject exists in the academy. If previously, the CurrenTSubject is not null, adds it to the CompletedSubjects array. If not, give error in console and do not set the CurrentSubject property.
## Exercise 2
